...
- **Real accounts, linked when you're ready.** Each person signs up on their own and gets a full solo budget. Generate an **invite code** (it shows its days left and can be revoked) and your partner either signs up with it or redeems it from their existing account — their solo history merges in as their personal envelopes, and from then on you share one household: joint budget, personal budgets, one running balance. The household names itself after the two of you ("Jacob and Sanya’s budget") until you type your own.
...
- **Themes** — light, dark, follow-system, or **auto** (follows the clock: light through the day, dark from 7pm to 7am), with eight accent palettes (violet, indigo, sky, teal, emerald, amber, rose, graphite). Per-device, so you can each pick your own look. Each member also picks their **own color** from a curated palette in Settings — it marks you on every chart, split, and avatar, and your partner can't grab the same one. The whole interface is deliberately **emoji-free** — clean type, real icons, and a folded-sheet mark.

### Plan — model it before you live it

- **The Plan page** (inspired by a standalone planner we loved enough to absorb) is a full modeling sandbox for the shared pot: enter gross pay **in your real rhythm** (yearly, monthly, twice a month, every other week, weekly), drag a 401(k) slider, list every paycheck deduction with its real tax treatment (§125 medical is FICA-free; plain pre-tax isn't) — and The Fold computes 2026 federal brackets, FICA with the wage-base cap, and a flat state rate to land on your **take-home pool**. Toggle *married filing jointly* vs *two single filers*, or flip to **manual rates** and give each person their own effective tax percentage (pre-filled from what the law computes).
- **Model a partner before they exist.** Solo on the server? Name a generic partner on the Plan page and model with their numbers — the moment they sign up with your invite, the plan adopts their real identity automatically and keeps everything you entered.
...
- **Contribution vs. allocation** — every planned category splits two ways at once: each person's *contribution* follows income share, and a per-line **benefit slider** sets each person's *allocation* — whose expense it really is. Any line can be a **fixed dollar amount or a percent of take-home** (the toggle converts in place so the dollars don't jump). A fairness check then shows what each of you puts in vs. what flows back, with the gap as a known, discussed number instead of a silent one. Personal allowances: equal dollars or proportional, with the trade-off spelled out.
...
- **The pool splits your way** — Living expenses (itemized by the planned budget), any number of custom set-aside buckets (Savings, Investments, Emergency fund, a house fund — each with an optional dollar goal and months-to-goal ETA), and personal allowances. Type exact percentages or dollar amounts, lock any slice, and the rest rebalance around it.
- **The plan meets reality, both directions** — one click pre-fills the whole model from your *tracked* incomes and 3-month category averages, and **Apply to budget** pushes the plan's numbers into the real envelopes (creating missing categories as needed) in one of two ways: **as your default from a chosen month onward** — untouched months fill themselves as they arrive, while hand-tuned months and all history stay exactly as they were — or **for one month only**, an override that later defaults never disturb. The Budget page shows a "Default since …" chip whenever a standing default is in force. The **plan vs. actually-happened** section scores each month against the model: spent vs. planned with a today marker, fixed bills that match show "✓ paid", and red only ever means *actually over* — never a scary extrapolation.
- **Pay modeled the way you're actually paid** — each person picks **salary** (annual figure) or **hourly** (rate × hours a week), plus a separate **pay frequency** (weekly, every other week, twice a month, monthly) that drives the per-paycheck numbers everywhere.
...
- **Splitwise-style expenses** — any expense can be split 50/50, by income, custom amounts, or "they owe it all"; a running balance shows who owes whom, with one-click settle-up records. Every new expense starts with a plain **"Comes out of"** choice — shared budget, or either person's personal money — which filters the category list and answers the split automatically (a personal expense lands on its owner's side of the settle-up no matter who swiped the card).
...
- **Lists** — to-dos, chores, groceries, wishlists (with prices and links), assignable to either of you, with due dates that read "Today" / "Tomorrow" / "3 days overdue". **Repeating chores** (every day, week, two weeks, or month) come back with the next date when you tick them off, keeping their weekday and never landing in the past.
...
- **Bank sync (SimpleFIN)** — connect your banks once at bridge.simplefin.org (no credentials ever touch The Fold), paste the setup token, and posted transactions pull themselves daily: auto-created accounts, FITID-style dedupe so nothing double-imports, balances flowing to Net worth, every pull an undoable batch landing in the classify queue. A Sync-now button when you can't wait.
...
- **Self-hosted account control** — the first account on the server is the admin; after that, signup is invite-only unless the admin opens registration (Settings → Server). Partner locked out? The admin can issue a one-time password from the Server card — no email server required. Change your name, email, and password in Settings → Your account — changing the password signs out every other device, and you can see how many devices are signed in (with one-click "sign out everywhere else"). Sign-in attempts are rate-limited, so a bot hammering your login page gets a locked door, not a guessing game.
...
- **Net worth (the Mint part)** — accounts for checking, savings, investments, retirement, property, and debts — joint or per-person — with dated balance snapshots, a household net-worth trend chart, and a by-person breakdown (assets, owed, net for each of you plus joint).
...
- **Dashboard** — month at a glance: shared + personal budget burn, who-owes-whom, net worth, next trip countdown, your open tasks — and a **"Needs a look"** card that only appears when something does: an envelope in the red, transactions without a category, bills posting this week, overdue chores, possible duplicates, or last month's review during the first week. Each line is a link to the fix.
...
- **Built to stay up** — if the server restarts mid-use, one slim banner says so and the app quietly reconnects and refetches (no dead white screens); an expired session bounces you to sign-in instead of silently failing; any save that fails shows a toast instead of quietly snapping back; pages show their outline while loading and offer a retry if the load fails. Sessions renew themselves while you keep using the app, cookies turn on `Secure` behind HTTPS automatically, hashed assets cache forever, and the container ships a Docker healthcheck.
